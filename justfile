...
build-app:
    {{sudo}} docker buildx build --platform="linux/amd64" -f Dockerfile.nitro -t lockhost-node-build-app .
    {{sudo}} docker rm -f lockhost-node-build-app > /dev/null 2>&1 || true
    {{sudo}} docker run --platform="linux/amd64" --name lockhost-node-build-app -v /var/run/docker.sock:/var/run/docker.sock lockhost-node-build-app
    mkdir -p dist
    {{sudo}} docker cp lockhost-node-build-app:/workspace/app.eif ./dist/ || true
    {{sudo}} docker cp lockhost-node-build-app:/workspace/app.pcr ./dist/ || true
    {{sudo}} docker rm -f lockhost-node-build-app > /dev/null  2>&1 || true
